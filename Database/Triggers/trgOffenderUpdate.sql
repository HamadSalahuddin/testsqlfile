...
Objects: TRIGGER
*/
CREATE TRIGGER [dbo].[trgOffenderUpdate]  ON dbo.Offender AFTER UPDATE
AS 
BEGIN
	...
	BEGIN 
		DECLARE @Deleted bit
		set @Deleted=(SELECT Deleted FROM inserted)
		UPDATE rprtEventsBucket2 SET OffenderDeleted=@Deleted WHERE  OffenderID=@OffenderID
		UPDATE rprtEventsBucket1 SET OffenderDeleted=@Deleted WHERE  OffenderID=@OffenderID
	END
END
GO
