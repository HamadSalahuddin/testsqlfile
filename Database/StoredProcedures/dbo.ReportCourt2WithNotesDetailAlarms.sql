...
Objects: PROCEDURE
*/
CREATE PROCEDURE [ReportCourt2WithNotesDetailAlarms]
...
UNION ALL
Select * From rprteventsbucket3 WITH (NOLOCK)
UNION ALL
Select * From rprteventsbucket4 WITH (NOLOCK)
) e
--FROM fnAllEvents()
Where e.EventDatetime >= @StartDate And e.EventDatetime <= @EndDate
And e.Offenderid = @offenderid And Alarmtype > 1
...
SELECT TOP 5000
	...
	(
		...
		END
          ...
          (CASE WHEN e.Gpsvalid != 1 and e.GpsValidSatellites != 1 OR e.eventid NOT IN (176,177,178,179,180,181,182,184,185,192,193,194,195)
          THEN 'Unavailable'
...
WHERE
	...
	and o.OffenderID = @OffenderID
    AND et.SO = 1 /*only the events that are available to So*/
ORDER BY 'Offender',e.EventTime asc, e.AlarmType
...
GO
GRANT EXECUTE ON [ReportCourt2WithNotesDetailAlarms] TO [db_dml]
GO
